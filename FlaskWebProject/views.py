...
def authorized():
    if request.args.get('state') != session.get("state"):
        # log unsuccessful log-in
        app.logger.error("************** error : unsuccessful log-in attempt, session state error **************")
        return redirect(url_for("home"))  # No-OP. Goes back to Index page

    if "error" in request.args:  # Authentication/Authorization failure
        # log unsuccessful log-in
        app.logger.error("************** error : unsuccessful log-in attempt, error in request.args **************")
    ...
    if request.args.get('code'):
        ...
        if "error" in result:
            # log unsuccessful log-in
            app.logger.error("************** error : unsuccessful log-in attempt, error in getting token from MSAL **************")
...
def _build_auth_url(authority=None, scopes=None, state=None):
        ...
        scopes or [],                       # from Config
        state = state or str(uuid.uuid4()), # for the session
        # redirect to our authorized() func
        redirect_uri = url_for('authorized', _external=True, _scheme='https')) 
